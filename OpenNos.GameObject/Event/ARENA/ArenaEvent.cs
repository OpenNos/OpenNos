                                          ...
                                          arenamembers.ToList().ForEach(o =>
                                              {
                                                  ...
                                                  Observable.Timer(TimeSpan.FromSeconds(1)).Subscribe(time =>
                                                  {
                                                      ...
                                                      Observable.Timer(TimeSpan.FromSeconds(5)).Subscribe(time2 =>
                                                      {
                                                          ...
                                                          o.Session.SendPacket(UserInterfaceHelper.Instance.GenerateTaP(2, arenateam, default(ArenaTeamType), false));
                                                          string groups = string.Empty;
                                                          IDisposable obs3 = Observable.Interval(TimeSpan.FromSeconds(5)).Subscribe(effect =>
                                                          {
                                                              arenateam.ToList().ForEach(eff => o.Session.SendPacket(eff.Session.Character.GenerateEff(eff.ArenaTeamType == ArenaTeamType.ERENIA ? 3013 : 3012)));
                                                          });
