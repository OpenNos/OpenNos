        ...
        private void NpcLife()
        {
            ...
            if (Target == -1)
            {
                if (Npc.IsHostile && Shop == null)
                {
                    ...
                    ClientSession session = MapInstance.Sessions.FirstOrDefault(s => MapInstance == s.Character.MapInstanceNode.Data && Map.GetDistance(new MapCell { X = MapX, Y = MapY }, new MapCell { X = s.Character.PositionX, Y = s.Character.PositionY }) < Npc.NoticeRange);
