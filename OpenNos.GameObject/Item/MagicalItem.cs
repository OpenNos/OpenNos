        ...
        public override void Use(ClientSession session, ref ItemInstance inv, bool delay = false, string[] packetsplit = null)
        {
            switch (Effect)
            {
                ...
                case 0:
                    ...
                    {
                        session.CurrentMapInstance?.Broadcast(ServerManager.GenerateEff(session.Character.CharacterId, EffectValue));
                ...
                case 14:
                    if ((EffectValue == 100 || EffectValue == 200) && session.Character.Dignity < 100 && !session.Character.IsVehicled)
                    {
                        ...
                        session.SendPacket(ServerManager.GenerateEff(session.Character.CharacterId, 49 - session.Character.Faction));
