    ...
    public class BattlePacketHandler : IPacketHandler
    {
        ...
        public void MultiTargetListHit(MultiTargetListPacket mutliTargetListPacket)
        {
            ...
            if (Session.Character.IsMuted())
            {
                ...
                Session.CurrentMap?.Broadcast(Session.Character.Gender == GenderType.Female
            ...
            {
                Session.SendPacket("cancel 0 0");
                Session.SendPacket(Session.Character.GenerateMsg(Language.Instance.GetMessageFromKey("CANT_ATTACKNOW"),
                    0));
                return;
            }
                ...
                mutliTargetListPacket.TargetsAmount == mutliTargetListPacket.Targets.Count)
            {
                foreach (MultiTargetListSubPacket subpacket in mutliTargetListPacket.Targets)
                {
                    ...
                    if (ski != null && ski.CanBeUsed() && Session.HasCurrentMap)
                    {
                        ...
                        if (mon != null &&
                            mon.IsInRange(Session.Character.MapX, Session.Character.MapY, ski.Skill.Range) &&
                            mon.CurrentHp > 0)
                        {
                        ...
                        Observable.Timer(TimeSpan.FromMilliseconds(ski.Skill.CastTime * 100))
                            .Subscribe(
                                o => { Session.SendPacket($"sr {subpacket.SkillCastId - 1}"); }
                            );
                    }
                }
            }
        ...
        public void TargetHit(int castingId, int targetId)
        {
            ...
            {
                CharacterSkill ski =
                    skills.FirstOrDefault(s => s.Skill?.CastId == castingId && s.Skill?.UpgradeSkill == 0);
                ...
                if (ski != null && Session.Character.Mp >= ski.Skill.MpCost)
                {
                    ...
                    if (ski.Skill.TargetType == 1 && ski.Skill.HitType == 1)
                    {
                        ...
                        CharacterSkill skillinfo =
                            Session.Character.Skills.GetAllItems()
                                .OrderBy(o => o.SkillVNum)
                                .FirstOrDefault(
                                    s =>
                    ...
                    else if (ski.Skill.TargetType == 0 && Session.HasCurrentMap)  // monster target
                    {
                        ...
                        {
                            if (Map.GetDistance(
                                    new MapCell { X = Session.Character.MapX, Y = Session.Character.MapY },
                                ...
                                ski.Skill.Range + monsterToAttack.Monster.BasicArea)
                            {
                                ...
                                Session.SendPacket(Session.Character.GenerateStat());
                                CharacterSkill characterSkillInfo = Session.Character.Skills.GetAllItems()
                                    .OrderBy(o => o.SkillVNum)
                                    .FirstOrDefault(
                                        s =>
                                ...
                                if (ski.Skill.TargetRange != 0)
                                {
                                    ...
                                    if (skillCombo != null)
                                    {
                                        ...
                                        IEnumerable<MapMonster> monstersInAoeRange =
            ...
            else
            {
                if (Session.Character.LastTransform.AddSeconds(3) > DateTime.Now)
                {
                    ...
                    Session.SendPacket(Session.Character.GenerateMsg(
                        Language.Instance.GetMessageFromKey("CANT_ATTACK"), 0));
                    return;
                }
                if (Session.Character.IsVehicled)
                {
        ...
        private void ZoneHit(int castingid, short x, short y)
        {
            ...
            if (characterSkill != null && characterSkill.CanBeUsed())
            {
                if (Session.Character.Mp >= characterSkill.Skill.MpCost)
                {
                    ...
                    Observable.Timer(TimeSpan.FromMilliseconds(characterSkill.Skill.CastTime * 100))
                        .Subscribe(
                            o =>
                            {
                                ...
                                if (Session.CurrentMap != null)
                                {
                                    IEnumerable<MapMonster> monstersInRange =
                                        Session.CurrentMap.GetListMonsterInRange(x, y, characterSkill.Skill.TargetRange)
                                            .ToList();
                    ...
                    Observable.Timer(TimeSpan.FromMilliseconds(characterSkill.Skill.CastTime * 100))
                        .Subscribe(
                            o => { Session.SendPacket($"sr {castingid}"); });
                }
                else
                {
                    Session.SendPacket(
