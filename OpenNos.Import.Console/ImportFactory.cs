        ...
        public void ImportNpcMonsterData()
        {
            foreach (string[] currentPacket in _packetList.Where(o => o[0].Equals("e_info") && o[1].Equals("10")))
            {
                if (currentPacket.Length > 25)
                {
                    ...
                    npcMonster.Concentrate = short.Parse(currentPacket[11]);
                    npcMonster.CriticalChance = byte.Parse(currentPacket[12]);
                    npcMonster.CriticalRate = short.Parse(currentPacket[13]);
                    npcMonster.DefenceUpgrade = byte.Parse(currentPacket[14]);
                    npcMonster.CloseDefence = short.Parse(currentPacket[15]);
                    npcMonster.DefenceDodge = short.Parse(currentPacket[16]);
                    npcMonster.DistanceDefence = short.Parse(currentPacket[17]);
                    npcMonster.DistanceDefenceDodge = short.Parse(currentPacket[18]);
                    npcMonster.MagicDefence = short.Parse(currentPacket[19]);
